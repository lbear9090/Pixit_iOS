...
@interface CVImgFilter ()

@end

@implementation CVImgFilter

-(UIImage *)processImage:(UIImage *)inputImage oldImage:(UIImage *)maskImage number:(int)effect sliderValueOne:(float)valueOne sliderValueTwo:(float)valueTwo
{
...
-(cv::Mat)sepiaConversion:(cv::Mat)inputMat
{
    ...
    cv::Mat M0 = cv::cvarrToMat(sepia);
    //cv::transform(inputMat, sepiaMat, (cv::Mat)(sepia));
    cv::transform(inputMat, sepiaMat, M0);
    inputMat.release();
